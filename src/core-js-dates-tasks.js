...
function isDateInPeriod(date, period) {
  ...
  return (
    dateToTime >= new Date(period.start) && dateToTime <= new Date(period.end)
  );
}
...
function getWeekNumberByDate(date) {
  const firstWeek = new Date(date.getFullYear(), 0, 1).getDay() || 7;
  const lastDay = date.getDay();
  let totalDays = date.getDate();
  for (let i = date.getMonth() - 1; i >= 0; i -= 1) {
    totalDays += new Date(date.getFullYear(), i, 0).getDate();
  }
  return totalDays > 7
    ? Math.ceil((totalDays - (7 - firstWeek + 1)) / 7) +
        (firstWeek === 1 && lastDay === 0 ? 0 : 1)
